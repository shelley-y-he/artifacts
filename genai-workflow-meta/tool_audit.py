"""
Claude Code PostToolUse hook — tool audit logger.
Appends one row per tool call to tool_audit.log in genai-workflow-meta/.
"""
...
LOG_PATH = r"G:\My Drive\Colab Notebooks\genai-workflow-meta\tool_audit.log"
...
def summarize_input(tool_name, tool_input):
    ...
    try:
        if tool_name == "Bash":
            ...
            # Fallback: first 150 chars of raw JSON
...
def main():
    try:
        ...
        sys.exit(0)  # Never block Claude on hook failure

    tool_name = data.get("tool_name", "Unknown")
    tool_input = data.get("tool_input", {})
    session_id = data.get("session_id", "")[:8]  # Short prefix only
